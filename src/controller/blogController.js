...
router.put("/update/:_id", verifyUser, async (req, res) => {
  try {
    const { BlogName, content } = req.body;
    const BlogUpdate = await updateBlog(
      { _id: req.params, status: "ACTIVE" },
      { BlogName: BlogName, content: content }
    );
...
router.delete("/delete", verifyUser, async (req, res) => {
  try {
    ...
    // Validate BlogId
    if (!BlogId) {
      return await makeResponse(res, 400, false, "BlogId is required");
    }
...
router.get("/user", verifyUser, async (req, res) => {
  try {
    const Blog = await getBlog({ _user: req.user._id, status: "ACTIVE" });
